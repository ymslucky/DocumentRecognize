...
class Renderer:
    ...
    def pdf_to_png(**param):
        ...
        with Document(file_path) as doc:
            ...
            [x, y, w, h] = param.get('img_rect', doc[0].rect)
    ...
    def pdf_to_png_multiprocess(**param):
        """渲染整个PDF文档为图片,多进程"""
        from system.task import TaskConfig as TC
        ...
        with Document(file_path) as doc:
            ...
            if step < int(os.getenv('PROCESS_PAGE')) or length < int(os.getenv('PROCESS_FILE_SIZE')):
                ...
                for index in range(0, length, step):
                    ...
                    res = TC.create_process_async(task=Renderer.pdf_page_to_png, param=param)
                    results.append(res)
            # 等待所有的处理结束
        ...
        with Document(file_path) as doc:
            ...
            if step < int(os.getenv('PROCESS_PAGE')) or length < int(os.getenv('PROCESS_FILE_SIZE')):
                ...
                for part_id in range(cpus):
                    ...
                    res = TC.create_process_async(task=Renderer.pdf_page_to_png, param=render_param)
                    results.append(res)
            # 等待所有的处理结束
            for index, item in enumerate(results):
                # 阻塞
